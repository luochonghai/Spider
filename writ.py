...
def get_text():
    ...
    fh = open("D:\\FDU\\Template\\NLP(ZhipengXie)\\Spider\\execu_case\\Doc_Id.txt","r",errors = 'ignore')
    doc_id = fh.readline()
    ...
    while doc_id:
        ...
        try:
            ...
            data = urllib.request.urlopen(codereq).read().decode(CODE_TYPE,"ignore")
            ...
            if(len(data) > 50):
                ws_raw = open("D:\\FDU\\Template\\NLP(ZhipengXie)\\Spider\\execu_case\\ws\\ws_"+str(counter)+".txt","a+",errors = 'ignore')
                counter += 1
                ws_raw.write(data)
                ws_raw.close()
                doc_id = fh.readline()
...
if __name__ =="__main__":
    ...
    for i in range(counter):
        file_path = "D:\\FDU\\Template\\NLP(ZhipengXie)\\Spider\\civil_case\\ws\\ws_"+str(i)+".txt"
